...
exports.deleteInvoice = exports.updateInvoiceStatus = exports.createInvoice = exports.invoiceById = exports.invoices = void 0;
...
const invoices = async (req, res) => {
    try {
        ...
        const merchant = await gridsession_db_1.GridUser.findOne({ email: merchantEmail });
        console.log('👤 Merchant lookup:', merchant ? `Found: ${merchant.email}` : 'Not found');
        if (!merchant) {
            return res.status(404).json({ message: 'Merchant not found' });
        }
        // Get invoices for this merchant only
        const invoices = await Invoice_1.Invoice.find({ merchant: merchant._id }).sort({ createdAt: -1 });
...
const invoiceById = async (req, res) => {
    try {
        const { id } = req.params;
        const invoice = await Invoice_1.Invoice.findById(id).populate('merchant', 'email fullName publicKey');
        if (!invoice) {
...
exports.invoiceById = invoiceById;
const createInvoice = async (req, res) => {
    try {
        const { amount, currency = 'USD', dueDate, payerEmail, payerName, description, items, merchantEmail, // We'll use this to find the merchant
         } = req.body;
...
const updateInvoiceStatus = async (req, res) => {
    try {
        ...
        const invoice = await Invoice_1.Invoice.findByIdAndUpdate(id, { status }, { new: true });
        if (!invoice) {
            return res.status(404).json({ message: 'Invoice not found' });
        }
        res.status(200).json(invoice);
